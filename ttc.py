...
╰──────────────────────────────────────╯"""))
    ...
    print(banner + may_anh + thong_tin)
def Delay(value):
    while not(value <= 1):
        value -= 0.123
        print(f'''{v}[{xanh}obi74z{v}] [{xanh}DELAY{v}] [{xanh}{str(value)[0:5]}{v}] [{vang}🔴    {v}]''', '               ', end = '\r')
        sleep(0.02)
        print(f'''{v}[{xanh}obi74z{v}] [{xanh}DELAY{v}] [{xanh}{str(value)[0:5]}{v}] [ {vang}🟠   {v}]''', '               ', end = '\r')
        sleep(0.02)
        print(f'''{v}[{xanh}obi74z{v}] [{xanh}DELAY{v}] [{xanh}{str(value)[0:5]}{v}] [  {vang}🟡  {v}]''', '               ', end = '\r')
                ...
                if "idpost" in getjob.text or "idfb" in getjob.text:
                    ...
                    for x in getjob.json():
                        ...
                        if nhanxu.get('status') == 'success':
                            ...
                            print(f'{do}[ \033[1;36m{stt}{do} ] {do}[ {vang}obi74z{do} ][ {xanh}{timejob}{do} ][ {vang}{type.upper()}{do} ][ {trang}{id_}{do} ][ {vang}{msg}{do} ][ {luc}{str(format(int(xu),","))} {do}]')
                            if stt % 10 == 0:
